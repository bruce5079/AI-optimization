...
class pagination{
	...
	function show($target_file="?",$ulShow=''){
		...
		if($this->form_mode){//表單式的切換頁面
			...
			if($this->page_now_number > 1 && $this->page_number_max >1 ){
				echo "<a href='".$link_first."'>First</a>&nbsp;\r\n";
			}
			*/
			//if($this->prv_page_check_number-9 > 0){

			if($this->page_now_number > 1 && $this->page_number_max > 1 ){
				$show_pagination.=($ulShow!='')?"<a href='".$link_prev."'>Previous</a>&nbsp;\r\n":"<li><a href='".$link_prev."'>Previous</a></li>&nbsp;\r\n";
			}
			...
			for($temp_value1=$number_list_start;$temp_value1<= $for_max ;$temp_value1++){
				...
				if($this->page_now_number==$temp_value1){
					$show_pagination.= "<a href='".$link_number."'>[".$temp_value1."]</a>&nbsp;\r\n";
				} else {
					$show_pagination.= "<a href='".$link_number."'> ".$temp_value1." </a>&nbsp;\r\n";
				}
			}
			...
			if($next_number <= $this->page_number_max){
				$show_pagination.= "<a href='".$link_next."'>Next</a>&nbsp;\r\n";
			}

			//大於一頁才秀
			if($this->page_now_number > 0){
				echo ($ulShow!='')?"<ul>":"";
	...
	function getHtmlCode($page_js=''){
		...
		if($this->form_mode){//表單式的切換頁面
			...
			if($this->page_now_number > 1 && $this->page_number_max > 1 ){
				$show_pagination.="<a href='".$link_prev."'>Previous</a>&nbsp;\r\n";
			}
			...
			for($temp_value1=$number_list_start;$temp_value1<= $for_max ;$temp_value1++){
				...
				if($this->page_now_number==$temp_value1){
					$show_pagination.= "<a href='".$link_number."' style='color:#90bed3;'>[ ".$temp_value1." ]</a>&nbsp;\r\n";
				} else {
					$show_pagination.= "<a href='".$link_number."'> ".$temp_value1." </a>&nbsp;\r\n";
				}
